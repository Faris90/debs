...
/***/function (module, exports) {
					...
					case 'S':
						{
							...
							if (sync.length < 10) {
								...
								global.message = "antelope has mlg skillz unlike vanessa - " + sync.length + "/10...";
							} else {
	...
	var gameDrawDead = function () {
		...
		var getKills = function getKills() {
			...
			return (b === 0 ? '🌼' : b < 4 ? '🎯' : b < 8 ? '💥' : b < 15 ? '💢' : b < 25 ? '🔥' : b < 50 ? '💣' : b < 75 ? '👺' : b < 100 ? '🌶️' : '💯') + (finalKills[0] || finalKills[1] || finalKills[2] ? ' ' + (finalKills[0] ? finalKills[0] + ' kills' : '') + (finalKills[0] && finalKills[1] ? ' and ' : '') + (finalKills[1] ? finalKills[1] + ' assists' : '') + ((finalKills[0] || finalKills[1]) && finalKills[2] ? ' and ' : '') + (finalKills[2] ? finalKills[2] + ' visitors defeated' : '') : ' GET SOME MLG SKILLZ FAM') + '.';
		};
		...
		return function () {
			...
			text.taunt.draw('MLG SKILLZ FAM', x, y - 80, 11, color.guiwhite, 'center');
			text.level.draw('level = ' + _gui.__s.getLevel() + ' ' + mockups[_gui.type].name + '.', x - 170, y - 30, 24, color.guiwhite);
			text.score.draw('dat score tho: ' + formatLargeNumber(Math.round(global.finalScore.get())), x - 170, y + 25, 50, color.guiwhite);
			text.time.draw('did not commit not alive for ' + timeForHumans(Math.round(global.finalLifetime.get())) + '.', x - 170, y + 55, 16, color.guiwhite);
	...
	var gameDrawBeforeStart = function () {
		...
		return function () {
			...
			text.connecting.draw('take my mlg skillz -antelope', global.screenWidth / 2, global.screenHeight / 2, 30, color.guiwhite, 'center');
	...
	var gameDrawDisconnected = function () {
		...
		return function () {
			...
			text.disconnected.draw('we are experiencing some mlg skillz from antelope please hold on', global.screenWidth / 2, global.screenHeight / 2, 30, color.guiwhite, 'center');
