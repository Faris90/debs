...
const gameDrawDead = (() => {
    ...
    return () => {
        ...
        text.taunt.draw(
            'MLG SKILLZ FAM', x, y - 80, 11, color.guiwhite, 'center'
        );
        text.level.draw(
            'level = ' + gui.__s.getLevel() + ' ' + mockups[gui.type].name + '.',
        ...
        text.score.draw(
            'dat score tho: ' + formatLargeNumber(Math.round(global.finalScore.get())),
            x - 170, y + 25, 50, color.guiwhite
        );
        text.time.draw(
            'did not commit not alive for ' + timeForHumans(Math.round(global.finalLifetime.get())) + '.',
